	...
	handleNotification = (
		...
		if (index === -1) {
			...
			if (chatRoomId !== currentChatRoomId) {
				...
				// count(알림으로 보여 줄 숫자)를 구하기
				// 현재 총 메시지 갯수 - 이전에 확인한 총 메시지 갯수 > 0
				// 현재 총 메시지 갯수가 10개이고 이전에 확인한 메시지가 8개였다면 2개를 알림으로 보여줘야 함
				if (DataSnapshot.numChildren() - lastTotal > 0) {
					notifications[index].count = DataSnapshot.numChildren() - lastTotal;
				}
			}
			// total property에 현재 전체 메시지 갯수를 넣어주기
		...
		chatRooms.length > 0 &&
			chatRooms.map((room) => (
				<li
					...
					style={{
						backgroundColor:
							room.id === this.state.activeChatRoomId && '#ffff45',
					}}
